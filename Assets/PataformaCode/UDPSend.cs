...
public class UDPSend : MonoBehaviour
{
    ...
    {
        // Obtener los ángulos de rotación del vehículo (en grados)
        Vector3 eulerAngles = vehicle.rotation.eulerAngles;

        // Normalizar los ángulos para que estén en el rango [-180, 180]
        float rotX = NormalizeAngle(eulerAngles.x);
        float rotY = NormalizeAngle(eulerAngles.y);
        float rotZ = NormalizeAngle(eulerAngles.z);

        // Ajustar A, B y C según la rotación en X y Z (puedes personalizar esto)
        // Ejemplo: Si el vehículo se inclina hacia adelante (rotX positivo), aumentar A y disminuir B
        // Si se inclina hacia los lados (rotZ), ajustar C

        // Factor de sensibilidad (ajusta según necesites)
        float sensibilidad = 0.5f;

        // Ajustar A y B según la inclinación hacia adelante/atrás (rotX)
        float targetA = 100f + (rotX * sensibilidad);
        float targetB = 100f - (rotX * sensibilidad);

        // Ajustar C según la inclinación lateral (rotZ)
        float targetC = 100f + (rotZ * sensibilidad);

        // Limitar los valores para que estén dentro del rango [0, 200]
        A = Mathf.Clamp(targetA, 0f, 200f);
        B = Mathf.Clamp(targetB, 0f, 200f);
        C = Mathf.Clamp(targetC, 0f, 200f);
    }
